...
export class ImageRunModal extends React.Component {
    ...
    loadImageDefaults = async (imageRef) => {
        try {
            ...
            // IMPORTANT: ExposedPorts can live under ContainerConfig if not present in Config
            ...
            console.log("[ImageRunModal] prefill parsed counts",
                `env=${parsedEnv.length}`, `vol=${parsedVol.length}`, `ports=${parsedPorts.length}`);

            if (!hasUserEnv && parsedEnv.length) {
                nextState.env = parsedEnv;
            }
            if (!hasUserVol && parsedVol.length) {
                nextState.volumes = parsedVol;
            }
            if (!hasUserPorts && parsedPorts.length) {
                nextState.publish = parsedPorts;
            }
            ...
            if (Object.keys(nextState).length) {
                ...
                console.log("[ImageRunModal] prefill: applying", nextState);
            }

            if (this._isMounted) {
                this.setState({ ...nextState, prefillLoading: false }, () => {
                    console.log("[ImageRunModal] after prefill setState -> env/vol/publish lengths:",
                        this.state.env.filter(Boolean).length,
                        this.state.volumes.filter(Boolean).length,
                        this.state.publish.filter(Boolean).length);
                });
            }
        } catch (e) {
